/**
 * Rules-based certainty alignment: detect mismatches and produce evidence flags.
 */

import type { ExtractedClaim, EvidenceFlag, EvidenceMapEntry } from "./types";
import { findEntry, getMentionedInterventions } from "./evidence-map";
...
export function detectFlags(
  ...
  for (let i = 0; i < claims.length; i++) {
    ...
    if (claim.claim_type === "lifespan_outcome" && claim.detected_certainty_level === "strong") {
      ...
      if (!hasHumanLifespan && mentioned.length > 0) {
        flags.push({
          ...
          claim_index: i,
          message: "Claim states lifespan outcome with strong certainty but human lifespan evidence is absent for mentioned interventions.",
        ...
        flags.push({
          type: "lifespan_certainty_mismatch",
          claim_index: i,
          message: "Claim states lifespan outcome with strong certainty; no human lifespan evidence in evidence map for this topic.",
          penalty: PENALTY.lifespan_certainty_mismatch,
        });
      }
    }
    ...
    if (claim.claim_type === "mechanistic" && claim.detected_certainty_level === "strong") {
      const impliesLifespan = /lifespan|longevity|live longer|extends life/i.test(claim.claim_text);
      if (impliesLifespan) {
        const hasHumanLifespan = mentioned.some((e) => e.human_lifespan_evidence);
        if (!hasHumanLifespan) {
          flags.push({
            ...
            claim_index: i,
            message: "Mechanism described with strong certainty and lifespan implication; human lifespan evidence not established.",
    ...
    const causalFraming = /\b(causes?|prevents?|extends?|reduces?)\b/i.test(claim.claim_text);
    if (causalFraming && claim.detected_certainty_level === "strong" && mentioned.length > 0) {
      ...
      if (!allSupported) {
        flags.push({
          ...
          claim_index: i,
          message: "Causal framing with strong certainty for interventions not in supported/established tier.",
    ...
    if (claim.detected_certainty_level === "strong" && mentioned.length > 0) {
      ...
      if (weakEvidence && !flags.some((f) => f.claim_index === i && f.type !== "minor_certainty_inflation")) {
        flags.push({
          type: "minor_certainty_inflation",
          claim_index: i,
          message: "Strong certainty wording for intervention(s) with experimental/emerging evidence only.",
