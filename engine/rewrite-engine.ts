/**
 * Rewrite raw response to align certainty with evidence; no moralizing or medical directives.
 */
...
function calculateEvidenceStrength(
  ...
  if (claim.claim_type === "healthspan_outcome" || claim.claim_type === "lifespan_outcome") {
...
function formatEvidenceContext(
  ...
  const claimsWithStrength = claims.map((c, idx) => ({
    ...
    intervention: map.find(e => c.claim_text.toLowerCase().includes(e.intervention.toLowerCase())),
  }));
  ...
  for (let i = 0; i < claimsWithStrength.length; i++) {
    ...
    const studyType = intervention ? (
      intervention.human_lifespan_evidence || intervention.human_healthspan_evidence !== "none" 
        ? (intervention.rct_presence !== "none" ? "Human RCTs" : "Human studies")
        : intervention.animal_lifespan_evidence !== "none" 
          ? "Animal studies"
          : "Limited evidence"
    ) : "Unknown";
    ...
    lines.push(`  Claim: "${claim.claim_text}"`);
    lines.push(`  Type: ${claim.claim_type} | Certainty: ${claim.detected_certainty_level} | Evidence tier: ${intervention?.evidence_label || "unknown"}${hasFlags ? " | ⚠️ HAS FLAGS - MUST SOFTEN" : ""}`);
    if (intervention) {
      lines.push(`  Human healthspan: ${intervention.human_healthspan_evidence} | RCTs: ${intervention.rct_presence} | Meta-analyses: ${intervention.meta_analysis_presence ? "Yes" : "No"}`);
    }
    lines.push("");
  }
...
export async function rewriteResponse(
  ...
  return llm.complete(REWRITE_SYSTEM, userMessage);
}
